...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      ...
      const formspreeRes = await fetch('https://formspree.io/f/xdablveq', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          admin_link: 'communitycarephysio.co.uk/#admin'
        })
      });
      const formspreeData = await formspreeRes.json();
      console.log('Formspree response:', formspreeRes.status, JSON.stringify(formspreeData));
